...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl md:text-3xl font-extrabold text-[#1a365d]">Tambah Akun Baru</h2>
        <p class="text-gray-500 text-sm mt-1">Masukkan data pengguna baru ke dalam sistem FITC.</p>
    </div>
...
<div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-6 md:p-10 w-full">
    <form action="{{ route('users.store') }}" method="POST">
        ...
        <!-- Grid untuk Nama dan Email sejajar di layar besar -->
        <div class="grid md:grid-cols-2 gap-6 mb-6">
            <!-- Nama -->
            <div>
                <label class="block text-[#1a365d] text-sm font-bold mb-2">Nama Lengkap</label>
                <input type="text" name="name" value="{{ old('name') }}" class="w-full px-5 py-4 rounded-xl border border-gray-200 focus:outline-none focus:border-[#1ba1e2] focus:ring-2 focus:ring-[#1ba1e2]/20 transition-all font-medium text-gray-700 bg-gray-50 focus:bg-white" placeholder="Contoh: Budi Santoso" required>
            </div>
            
            <!-- Email -->
            <div>
                <label class="block text-[#1a365d] text-sm font-bold mb-2">Alamat Email</label>
                <input type="email" name="email" value="{{ old('email') }}" class="w-full px-5 py-4 rounded-xl border border-gray-200 focus:outline-none focus:border-[#1ba1e2] focus:ring-2 focus:ring-[#1ba1e2]/20 transition-all font-medium text-gray-700 bg-gray-50 focus:bg-white" placeholder="budi@contoh.com" required>
            </div>
        </div>
        
        <!-- Input Password dengan Alpine.js (Show/Hide) -->
        <div class="mb-8 w-full" x-data="{ showPassword: false }">
            <label class="block text-[#1a365d] text-sm font-bold mb-2">Password</label>
            <div class="relative">
                <input :type="showPassword ? 'text' : 'password'" name="password" class="w-full px-5 py-4 pr-14 rounded-xl border border-gray-200 focus:outline-none focus:border-[#1ba1e2] focus:ring-2 focus:ring-[#1ba1e2]/20 transition-all font-medium text-gray-700 bg-gray-50 focus:bg-white" placeholder="Minimal 6 karakter" required>
        ...
        <div class="flex flex-col sm:flex-row gap-4 border-t border-gray-100 pt-8 mt-4">
            <button type="submit" class="bg-gradient-to-r from-[#1a365d] to-[#1ba1e2] hover:from-[#1ba1e2] hover:to-[#5bc0de] text-white font-bold py-4 px-12 rounded-xl transition-all shadow-lg transform hover:-translate-y-1 w-full sm:w-auto text-center">
                Simpan Akun
            </button>
