...
<div class="max-w-full bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-100">
    ...
    <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-100">
        <div>
            <h2 class="text-2xl font-extrabold text-[#1a365d]">Tambah Master Fasilitator</h2>
            <p class="text-gray-500 text-sm mt-1">Lengkapi formulir di bawah untuk mendaftarkan fasilitator baru.</p>
        </div>
    ...
    <form action="{{ route('master.fasilitator.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama_fasilitator" value="{{ old('nama_fasilitator') }}" required class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1ba1e2] font-medium text-sm" placeholder="Contoh: Dr. H. Ahmad Fauzi, M.Kes">
            </div>

            <!-- Profesi -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Profesi</label>
                <input type="text" name="profesi_fasilitator" value="{{ old('profesi_fasilitator') }}" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1ba1e2] font-medium text-sm" placeholder="Contoh: Dokter Spesialis / Widyaiswara">
            </div>

            <!-- Jabatan -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Jabatan</label>
                <input type="text" name="jabatan_fasilitator" value="{{ old('jabatan_fasilitator') }}" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1ba1e2] font-medium text-sm" placeholder="Contoh: Kepala Bagian Diklat">
            </div>

            <!-- Tempat Kerja -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Tempat Kerja</label>
                <input type="text" name="tempat_kerja_fasilitator" value="{{ old('tempat_kerja_fasilitator') }}" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1ba1e2] font-medium text-sm" placeholder="Contoh: RSUD Simpang Lima Gumul Kediri">
            </div>

            <!-- Pendidikan Terakhir -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Pendidikan Terakhir</label>
                <input type="text" name="pendidikan_terakhir_fasilitator" value="{{ old('pendidikan_terakhir_fasilitator') }}" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1ba1e2] font-medium text-sm" placeholder="Contoh: S3 Kedokteran Universitas Airlangga">
            </div>

            <!-- Upload Dokumen -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Dokumen Pendukung (PDF / DOC / DOCX)</label>
                <div class="relative border-2 border-dashed border-gray-200 rounded-2xl p-4 bg-gray-50 hover:bg-gray-100/50 transition-all text-center cursor-pointer">
                    ...
                    <div class="flex flex-col items-center justify-center space-y-1">
                        ...
                        <p id="doc-label" class="text-sm font-bold text-gray-600">Klik untuk unggah dokumen</p>
            ...
            <!-- Upload Foto 3x4 -->
            <div>
                <label class="block font-bold text-sm text-gray-700 mb-2">Foto 3x4 (JPG / PNG)</label>
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative flex-1 border-2 border-dashed border-gray-200 rounded-2xl p-3 bg-gray-50 hover:bg-gray-100/50 transition-all text-center cursor-pointer">
                        ...
                        <p id="foto-label" class="text-xs font-bold text-gray-600">Pilih Foto 3x4</p>
            ...
            <div class="md:col-span-2">
                <label class="block font-bold text-sm text-gray-700 mb-2">Materi Yang Diampu (Pilih satu atau lebih)</label>
                <select name="materi_ids[]" id="choices-materi" multiple class="w-full">
                    @foreach($materis as $m)
                        <option value="{{ $m->id }}">{{ $m->nama_materi }} (Ambang Batas: {{ $m->ambang_batas }})</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
            ...
            <button type="submit" class="bg-[#1a365d] hover:bg-[#1ba1e2] text-white px-8 py-3 rounded-xl font-bold text-sm transition-all shadow-lg">Simpan Fasilitator</button>
        </div>
    </form>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(element) {
            new Choices(element, {
                ...
                placeholder: true,
                placeholderValue: 'Pilih materi yang diampu...',
    ...
    function validateAndPreviewDoc(input, labelId) {
        if (input.files && input.files[0]) {
            ...
            if (file.size > maxSize) {
                ...
                input.value = '';
                document.getElementById(labelId).innerText = 'Klik untuk unggah dokumen';
                return;
            }
