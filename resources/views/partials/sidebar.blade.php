...
<aside :class="sidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full w-0 lg:w-0 lg:-translate-x-full'" 
    ...
    <div class="w-72 flex flex-col h-full">
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
            ...
            <!-- Kategori: Main Menu -->
<!-- 0. Dashboard -->
<div x-show="!menuSearch || 'dashboard'.includes(menuSearch.toLowerCase())">
    <p class="px-2 text-[10px] font-bold tracking-widest text-slate-500 uppercase mb-2 mt-1">Main Menu</p>
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3.5 {{ request()->routeIs('dashboard') ? 'bg-blue-600 shadow-lg shadow-blue-900/50 text-white' : 'hover:bg-slate-800 text-slate-300' }} rounded-xl font-bold transition-all transform hover:-translate-y-0.5">
        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 13h4v8H3v-8zm7-9h4v17h-4V4zm7 5h4v12h-4V9z"></path>
        </svg>
        Dashboard
    </a>
</div>
...
<div x-show="!menuSearch || 'manajemen akun'.includes(menuSearch.toLowerCase())">
    <a href="{{ route('users.index') }}" class="flex items-center gap-3 px-4 py-3.5 mt-2 {{ request()->routeIs('users.*') ? 'bg-blue-600 shadow-lg shadow-blue-900/50 text-white' : 'hover:bg-slate-800 text-slate-300' }} rounded-xl font-bold transition-all transform hover:-translate-y-0.5">
        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
